...
describe("POST /users", function () {
  test("works for admin: create non-admin", async function () {
    ...
    test("create non-admin, unauth for non-admin", async function () {
      const resp = await request(app)
          .post("/users")
          .send({
            username: "u-new",
  ...
  test("create admin, unauth for non-admin", async function () {
    const resp = await request(app)
        .post("/users")
        .send({
          username: "u-new",
...
describe("GET /users/:username", function () {
  ...
  test("works for not current user, admin", async function () {
    const resp = await request(app)
        .get(`/users/u1`)
        .set("authorization", `Bearer ${u4Token}`);
    expect(resp.body).toEqual({
